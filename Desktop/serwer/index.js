...
app.post("/zlecenia", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const query = `
      INSERT INTO przejazdy (pasazer_id, kierowca_id, dystans_km, trasa_przejazdu, cena, data_zamowienia, status_id, chat_status_id)
      VALUES (?, ?, ?, ?, ?, NOW(), ?, 1)
    `;
...
app.get("/chats", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Jeśli używasz szyfrowania:
    const data = await encryptData(rows);

    // Jeśli nie szyfrujesz, zamiast powyższego zrób po prostu:
    // res.json(rows);
...
app.get('/chats/:rideId', authenticateToken, async (req, res) => {
  const rideId = req.params.rideId;
  const conn = await connectDB();
  const [rows] = await conn.execute(
    'SELECT chat_status_id AS status FROM przejazdy WHERE id = ?',
    [rideId]
  );
  await conn.end();
  if (!rows.length) return res.status(404).json({ message: 'Nie znaleziono czatu' });
  res.json({ status: rows[0].status });
});

app.get('/admin/chats', authenticateToken, authorizeAdmin, async (req, res) => {
  const conn = await connectDB();
  const [rows] = await conn.execute(`
    SELECT 
      p.id                AS rideId,
      p.data_zamowienia,
      up.imie     AS otherName,
      p.chat_status_id    AS status
    FROM przejazdy p
    JOIN uzytkownicy up ON 
      (up.id = p.pasazer_id OR up.id = p.kierowca_id)
    ORDER BY p.data_zamowienia DESC
  `);
  await conn.end();
  res.json(rows);
